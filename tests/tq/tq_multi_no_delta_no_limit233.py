...
logger = setup_logger(file_name=os.path.join(os.path.dirname(__file__), 'in_day_trade_no_delta_no_limit233'),
                           name='trade_pp',
                           log_level=logging.DEBUG,
                           backtesing=True)
price_logger = setup_logger(file_name=os.path.join(os.path.dirname(__file__), 'in_day_price_no_delta_no_limit233'),
                      name='price_pp',
                      log_level=logging.DEBUG,
                      backtesing=True)
...
def cal_open_signal():
    ...
    if (now_hour == 9 and now_minute == 18) or (now_hour == 21 and now_minute == 18):
        # 前分钟不开仓
    ...
    if (now_hour == 9 and now_minute <= 18) or (now_hour == 13 and now_minute <= 40) or (now_hour == 21 and now_minute <= 18) or (now_hour == 11 and now_minute >= 15) or (1330 <= int_time <= 1500) or (2245 <= int_time <= 2300):
        # 前分钟不开仓
        return 0
    ...
    if ticks.iloc[-1].bid_price1 > ma20.ma.iloc[-1]:
        ...
        if kline_form is not None and kline_form.direction == Direction.LONG:
            ...
            if ((time_condition and stop_condition) or (price_int_time == 2118 or price_int_time == 918)) and price_condition:
