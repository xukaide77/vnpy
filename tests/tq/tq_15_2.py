...
logger = setup_logger(file_name=os.path.join(os.path.dirname(__file__), 'order_flow_trade152'),
                           name='trade152',
...
api = TqApi(TqSim(20000), backtest=TqBacktest(start_dt=date(2020, 8, 8), end_dt=date(2020, 10, 30)), web_gui=True, auth="xukaide77,xk82513994")
...
def cal_open_signal(delta, poc_price, red_or_green, tiny_flag):
    ...
    if ((main_long_condition and limit_open_condition_delta and (limit_open_condition_delta_ratio or limit_open_condition_volume) and limit_long_condition_poc and limit_long_condition_heap) or multi_heap_long_condition):
        stop_price = min(demand_heap[0].min_price - price_tick, ticks.iloc[-1].ask_price1 - 10 * price_tick)
    ...
    if ((main_short_condition and limit_open_condition_delta and (limit_open_condition_delta_ratio or limit_open_condition_volume) and limit_short_condition_poc and limit_short_condition_heap) or multi_heap_short_condition) and limit_short_condition_stop:
        stop_price = max(supply_heap[-1].max_price + price_tick, ticks.iloc[-1].bid_price1 + 10 * price_tick)
