...
logger = setup_logger(file_name=os.path.join(os.path.dirname(__file__), 'order_flow_trade15'),
                           name='trade15',
                           log_level=logging.DEBUG,
                           backtesing=True)
price_logger = setup_logger(file_name=os.path.join(os.path.dirname(__file__), 'order_flow_price15'),
                      name='price15',
                      log_level=logging.DEBUG,
                      backtesing=True)
...
api = TqApi(TqSim(20000), backtest=TqBacktest(start_dt=date(2020, 11, 1), end_dt=date(2020, 11, 6)), web_gui=True, auth="xukaide77,xk82513994")
...
# sym = 'CFFEX.IC2011'
ex, sy = sym.split('.')
min5_kline = api.get_kline_serial(sym, trade_period * 60, data_length=45)
# 映射交易合约
...
def CoreModule():
    ...
    min5_kline['ma40_MAIN.color'] = 'blue'
    ma_trend = 1 if ma20.iloc[-1].ma >= ma40.iloc[-1].ma else -1
    # 画图
    ...
    cal_open_signal(delta, poc_price, red_or_green, tiny_flag, ma_trend)
...
def cal_open_signal(delta, poc_price, red_or_green, tiny_flag, ma_trend):
    ...
    if (now_hour == 9 and now_minute <= 15) or (now_hour == 13 and now_minute <= 45) or (now_hour == 21 and now_minute <= 15) or (now_hour == 11 and now_minute >= 15) or (now_hour == 14 and now_minute >= 45) or (100 <= can_time(now_hour, now_minute) < 259):
        # 前5分钟不开仓
        return 0
    main_long_condition = len(demand_heap) > 0 and len(supply_heap) == 0 and red_or_green > 0 and delta > 0 and ma_trend == 1
    main_short_condition = len(supply_heap) > 0  and len(demand_heap) == 0 and red_or_green < 0 and delta < 0 and ma_trend == -1
    ...
    limit_long_condition_poc = True if not (abs(am.high[-1] - am.close[-1]) > 0.5 * abs(am.close[-1] - am.open[-1])) else poc_price < max(am.close[-1], am.open[-1]) + 2 * price_tick and poc_price < am.high[-1] - 2 * price_tick
    limit_short_condition_poc = True if not (abs(am.low[-1] - am.close[-1]) > 0.5 * abs(am.close[-1] - am.open[-1])) else poc_price > min(am.close[-1], am.open[-1]) - 2 * price_tick and poc_price > am.low[-1] + 2 * price_tick
    limit_long_condition_heap = len(demand_heap) > 0 and (True if not (abs(am.high[-1] - am.close[-1]) > 0.5 * abs(am.close[-1] - am.open[-1])) else (demand_heap[0].max_price < max(am.close[-1], am.open[-1]) + 2 * price_tick and demand_heap[0].max_price < am.high[-1] - 2 * price_tick))
    limit_short_condition_heap = len(supply_heap) > 0 and (True if not (abs(am.low[-1] - am.close[-1]) > 0.5 * abs(am.close[-1] - am.open[-1])) else (supply_heap[len(supply_heap) - 1].min_price > min(am.close[-1], am.open[-1]) - 2 * price_tick and supply_heap[len(supply_heap) - 1].min_price > am.low[-1] + 2 * price_tick))
    # 多个失衡堆积,开仓
    ...
    if ((main_long_condition and limit_open_condition_delta and (limit_open_condition_delta_ratio or limit_open_condition_volume) and limit_long_condition_poc and limit_long_condition_heap) or multi_heap_long_condition):
        stop_price = min(demand_heap[0].min_price - price_tick, ticks.iloc[-1].ask_price1 - 5 * price_tick)
        if long_stop_price > 0:
            ...
            condition_open_long_price = demand_heap[0].min_price + open_stop_tick * price_tick
            log('设置开多条件价至{}'.format(condition_open_long_price))
    if ((main_short_condition and limit_open_condition_delta and (limit_open_condition_delta_ratio or limit_open_condition_volume) and limit_short_condition_poc and limit_short_condition_heap) or multi_heap_short_condition) and limit_short_condition_stop:
        stop_price = max(supply_heap[-1].max_price + price_tick, ticks.iloc[-1].bid_price1 + 5 * price_tick)
        if short_stop_price > 0:
            ...
            long_stop_price = 0
            open_signal = -1
        elif limit_short_condition_stop:
            log('设置开仓止损位至{}'.format(stop_price))
            short_stop_price = stop_price
            long_stop_price = 0
            open_signal = -1
        else:
            condition_open_short_price = supply_heap[-1].max_price - open_stop_tick * price_tick
            log('设置开空条件价至{}'.format(condition_open_short_price))
...
# ---------------------交易模块----------------------#
def trade_mode():
    global heap_flag, Up_Aprice, Dn_Bprice, total_vol, poc_price, PG, PGG, PNN, Max_price, Min_price, M_minprice, W_maxprice, D_Low, open_signal, stop_signal, long_stop_price, short_stop_price, condition_open_long_price, condition_open_short_price
...
def close_and_clearn():
    global is_trading, total_ask_vol, total_buy_vol, sumB, sumA, poc_price, poc_Ask, poc_Bid, poc_vol, last_bar, is_tq_min5_ok, bg, am, condition_open_long_price, condition_open_short_price
    if (is_trading == 1):
        trySendOrder(lots, 0, 1)
        trySendOrder(lots, 0, -1)
        is_trading = 0
