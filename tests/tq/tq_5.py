...
logger = setup_logger(file_name=os.path.join(os.path.dirname(__file__), 'order_flow_trade5'),
                           name='trade5',
                           log_level=logging.DEBUG,
                           backtesing=True)
price_logger = setup_logger(file_name=os.path.join(os.path.dirname(__file__), 'order_flow_price5'),
                      name='price5',
                      log_level=logging.DEBUG,
                      backtesing=True)
...
# api = TqApi(TqSim(20000), backtest=TqBacktest(start_dt=date(2020, 8, 5), end_dt=date(2020, 8, 5)), web_gui=True, auth="xukaide77,xk82513994")
api = TqApi(TqSim(20000), backtest=TqBacktest(start_dt=datetime(2020, 8, 5, 14, 30), end_dt=datetime(2020, 8, 5, 15, 0)), web_gui=True, auth="xukaide77,xk82513994")
...
sym = "SHFE.ag2012"
ex, sy = sym.split('.')
min5_kline = api.get_kline_serial(sym, trade_period * 60, data_length=10)
# 映射交易合约
...
def cal_open_signal(delta, poc_price, red_or_green, tiny_flag):
    ...
    if ((main_long_condition and limit_open_condition_delta and (limit_open_condition_delta_ratio or limit_open_condition_volume) and limit_long_condition_poc and limit_long_condition_heap) or multi_heap_long_condition) and limit_long_condition_stop:
