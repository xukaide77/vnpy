...
logger = setup_logger(file_name=os.path.join(os.path.dirname(__file__), 'order_flow_trade153'),
                           name='trade153',
                           log_level=logging.DEBUG,
                           backtesing=True)
price_logger = setup_logger(file_name=os.path.join(os.path.dirname(__file__), 'order_flow_price153'),
                      name='price153',
                      log_level=logging.DEBUG,
                      backtesing=True)
...
api = TqApi(TqSim(20000), backtest=TqBacktest(start_dt=date(2020, 8, 10), end_dt=date(2020, 8, 14)), web_gui=True, auth="xukaide77,xk82513994")
...
def cal_open_signal(delta, poc_price, red_or_green, tiny_flag):
    ...
    if (now_hour == 9 and now_minute <= 15) or (now_hour == 13 and now_minute <= 45) or (now_hour == 21 and now_minute <= 15) or (now_hour == 11 and now_minute >= 15) or (now_hour == 14 and now_minute >= 45) or (100 <= can_time(now_hour, now_minute) < 259):
        # 前5分钟不开仓
        return 0
    if abs(am.close[-1] - am.open[-1]) < open_tick_limit:
        log(f'k线变动不超过{open_tick_limit}个价位，不计算开仓')
        return
    ...
    if ((main_short_condition and limit_open_condition_delta and (limit_open_condition_delta_ratio or limit_open_condition_volume) and limit_short_condition_poc and limit_short_condition_heap) or multi_heap_short_condition):
