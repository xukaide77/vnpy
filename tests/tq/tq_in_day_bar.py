...
api = TqApi(TqSim(10000), backtest=TqBacktest(start_dt=date(2020, 11, 1), end_dt=date(2020, 12, 15)), web_gui=True, auth="xukaide77,xk82513994")
...
def cal_key_price():
    ...
    if (now_hour == 9 and now_minute == 3) or (now_hour == 21 and now_minute == 3):
        ...
        key_price.today_high = min5_kline.iloc[-1].open
        ...
        if min(min5_kline.iloc[-1].open, min5_kline.iloc[-1].close) <= key_price.open_low:
            ...
            rise = round((key_price.open_high - key_price.today_high) / day_kline.iloc[-1].open * 100, 2)
            fall = round((key_price.open_low - key_price.today_high) / day_kline.iloc[-1].open * 100, 2)
            api.draw_text(min5_kline, rise, x=-1, y=key_price.open_high + 8 * price_tick, color='red')
            api.draw_text(min5_kline, fall, x=-1, y=key_price.open_low - 5 * price_tick, color='green')
...
def cal_stop_signal():
    ...
    if long_entry_price > 0:
        wait_period += 1
        if min5_kline.iloc[-1].close < ma10.ema.iloc[-1] and min5_kline.iloc[-1].close > long_entry_price and wait_period > 3:
            stop_signal = 1
            log('跌破10日线止盈')
        ...
        wait_period += 1
        if min5_kline.iloc[-1].close > ma10.ema.iloc[-1] and min5_kline.iloc[-1].close < short_entry_price and wait_period > 3:
            stop_signal = -1
            log('升破10日线止盈')
        # elif vrsi.vrsi.iloc[-2] > 75 and vrsi.vrsi.iloc[-1] < 75 and wait_period > 3:
        #     stop_signal = -1
