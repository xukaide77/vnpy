...
api = TqApi(TqSim(10000), backtest=TqBacktest(start_dt=date(2020, 11, 6), end_dt=date(2020, 11, 6)), web_gui=True, auth="xukaide77,xk82513994")
# api = TqApi(TqSim(10000), backtest=TqBacktest(start_dt=datetime(2020, 11, 2, 9, 0), end_dt=datetime(2020, 11, 2, 15, 0)), web_gui=True, auth="xukaide77,xk82513994")
...
def cal_open_signal(delta, poc_price, red_or_green, tiny_flag):
    ...
    if short_trend == 1 and ticks.iloc[-1].bid_price1 > ma20.ma.iloc[-1]:
        if kline_form is not None and kline_form.type == KlineType.BOTTOM:
            ...
            log(f'short_trend:{short_trend},stop_condition:{stop_condition},kline_form.stop_price:{kline_form.stop_price},ma:{ma20.ma.iloc[-1]}')
            if stop_condition:
                open_signal = 1
            ...
            log(f'short_trend:{short_trend},time_condition:{time_condition},stop_condition:{stop_condition},ma20:{ma20.ma.iloc[-1]}')
            if time_condition and stop_condition and price_condition:
                ...
                short_stop_price = 0
    elif short_trend == -1 and ticks.iloc[-1].ask_price1 < ma20.ma.iloc[-1]:
        if kline_form is not None and kline_form.type == KlineType.TOP:
            ...
            log(f'short_trend:{short_trend},stop_condition:{stop_condition},kline_form.stop_price:{kline_form.stop_price},ma20:{ma20.ma.iloc[-1]}')
            if stop_condition:
                open_signal = -1
...
def top_or_bottom(int_time, ma5, ma10, ma20):
    ...
    if top_condition1:
        ...
        stop_price = min5_kline.iloc[middle_index].close
        api.draw_text(min5_kline, '回调', x=middle_index, y=min5_kline.iloc[middle_index].high + 8 * price_tick, color='red')
        return KlineForm(stop_price, KlineType.TOP, int_time)
    ...
    if bottom_condition1:
        ...
        stop_price = min5_kline.iloc[middle_index].close
        api.draw_text(min5_kline, '回调', x=middle_index, y=min5_kline.iloc[middle_index].low - 11 * price_tick, color='blue')
        return KlineForm(stop_price, KlineType.BOTTOM, int_time)
    return None
...
def close_and_clearn():
    global is_trading, total_ask_vol, total_buy_vol, sumB, sumA, poc_price, poc_Ask, poc_Bid, poc_vol, condition_open_long_price, condition_open_short_price, short_trend, last_kline_form, current_kline_form, shock_down_limit, shock_upper_limit, last_poc_price, profit_time_left
    if (is_trading == 1):
        trySendOrder(lots, 0, 1)
        trySendOrder(lots, 0, -1)
        is_trading = 0
