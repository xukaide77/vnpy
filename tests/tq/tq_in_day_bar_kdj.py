...
api = TqApi(TqSim(10000), backtest=TqBacktest(start_dt=date(2020, 10, 1), end_dt=date(2020, 12, 15)), web_gui=True, auth="xukaide77,xk82513994")
...
ex, sy = sym.split('.')
small_kline = api.get_kline_serial(sym, trade_period * 60, data_length=20)
min5_kline = api.get_kline_serial(sym, trade_period * 60 * 60, data_length=kline_length)
...
def cal_key_price():
    ...
    now_hour = now_datetime.hour
    now_minute = now_datetime.minute
    kd = ta.KD(min5_kline, 4, 3, 3)
    if kd.k.iloc[-2] < kd.d.iloc[-2] and kd.k.iloc[-1] > kd.d.iloc[-1]:
        if kd.k.iloc[-1] < 50:
            key_price.jincha_count += 1
        else:
            key_price.jincha_count = 0
    elif kd.k.iloc[-2] > kd.d.iloc[-2] and kd.k.iloc[-1] < kd.d.iloc[-1]:
        if kd.k.iloc[-1] > 50:
            key_price.sicha_count += 1
        else:
            key_price.sicha_count = 0
    ma100 = ta.MA(min5_kline, 100).ma.iloc[-1]
    if min5_kline.iloc[-1].close > ma100:
        key_price.trend = 1
    elif min5_kline.iloc[-1].close < ma100:
        key_price.trend = -1
...
def cal_stop_signal():
    ...
    ma20 = ta.MA(min5_kline, 20)
    if long_entry_price > 0:
        wait_period += 1
        if min5_kline.iloc[-1].close < ma20.ma.iloc[-1] and min5_kline.iloc[-1].close > long_entry_price and wait_period > 10:
            stop_signal = 1
            log('跌破20日线止盈')
        ...
        # elif wait_period >= max_wait_period and min5_kline.iloc[-1].close < long_entry_price:
        ...
        wait_period += 1
        if min5_kline.iloc[-1].close > ma20.ma.iloc[-1] and min5_kline.iloc[-1].close < short_entry_price and wait_period > 10:
            stop_signal = -1
            log('升破20日线止盈')
        elif wait_period >= max_wait_period and min5_kline.iloc[-1].close > short_entry_price:
...
def Ctrltime():
    ...
    is_trading = 0
    if (900 <= can_time(hour_new, minute_new) < 1130 or 1300 <= can_time(hour_new, minute_new) < 1500):
        is_trading = 1
    if (NT == 1):
        if ((2100 <= can_time(hour_new, minute_new) < 2300)):
            is_trading = 1  # 夜盘交易时间段21-23
    elif (NT == 2):
        if ((2100 <= can_time(hour_new, minute_new) < 2357) or (0 <= can_time(hour_new, minute_new) < 300)):
...
while True:
    try:
        ...
        if api.is_changing(min5_kline.iloc[-1], ["volume"]) and min5_kline.iloc[-1].volume > 0:
            ...
            tq_now = tafunc.time_to_datetime(small_kline.iloc[-1].datetime)
            # 核心模块
            # price_logger.debug(f'天勤k线:time:{tq_now}, open:{small_kline.iloc[-1].open}, close:{small_kline.iloc[-1].close}, high:{small_kline.iloc[-1].high}, low:{small_kline.iloc[-1].low}, volume:{small_kline.iloc[-1].volume}')
            trade_mode()
            Ctrltime()
