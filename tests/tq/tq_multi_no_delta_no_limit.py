...
logger = setup_logger(file_name=os.path.join(os.path.dirname(__file__), 'in_day_trade_no_delta_no_limit'),
                           name='trade_pp',
                           log_level=logging.DEBUG,
                           backtesing=True)
price_logger = setup_logger(file_name=os.path.join(os.path.dirname(__file__), 'in_day_price_no_delta_no_limit'),
                      name='price_pp',
                      log_level=logging.DEBUG,
                      backtesing=True)
...
profit_tick = 20  # 止盈跳
big_profit_tick = 99  # 止盈跳
loss_tick = 20  # 止损跳
trade_period = 3 #单位分钟
max_wait_period = 5
time_limit = 99 # 距离分型k线根数
...
def cal_open_signal():
    global long_stop_price, short_stop_price, open_signal, wait_period, short_trend, current_kline_form, shock_upper_limit, shock_down_limit, profit_time_left, open_ready_price, middle_trend, last_open_kline_form
    # 画一次指标线
    ...
    if ticks.iloc[-1].bid_price1 > ma20.ma.iloc[-1]:
        ...
        if current_kline_form is not None and current_kline_form.direction == Direction.LONG:
            if long_entry_price > 0:
                ...
                return
            stop_condition = (ticks.iloc[-1].ask_price1 - loss_tick * price_tick) < current_kline_form.stop_price or ((ticks.iloc[-1].ask_price1 - loss_tick * price_tick) < ma20.ma.iloc[-1] and middle_trend > 0) or last_open_kline_form is None
        ...
        if current_kline_form is not None and current_kline_form.direction == Direction.SHORT:
            if short_entry_price > 0:
                ...
                return
            stop_condition = (ticks.iloc[-1].bid_price1 + loss_tick * price_tick) > current_kline_form.stop_price or ((ticks.iloc[-1].bid_price1 + loss_tick * price_tick) > ma20.ma.iloc[-1] and middle_trend < 0) or last_open_kline_form is None
...
def top_or_bottom(int_time, ma5, ma10, ma20):
    ...
    top_condition4 = min5_kline.iloc[last_index].open - min5_kline.iloc[last_index].close > 5 * price_tick and min5_kline.iloc[middle_index].close < min5_kline.iloc[middle_index].open and (min5_kline.iloc[first_index].close - min5_kline.iloc[first_index].open) >= 0 and ma5.ma.iloc[first_index] < ma10.ma.iloc[first_index]
    # 明显的顶分型快速开仓
    top_condition5 = min5_kline.iloc[last_index].open - min5_kline.iloc[last_index].close > 5 * price_tick and (min5_kline.iloc[last_index].close < min5_kline.iloc[middle_index].open or min5_kline.iloc[last_index].open - min5_kline.iloc[last_index].close > 10 * price_tick) and (min5_kline.iloc[middle_index].close - min5_kline.iloc[middle_index].open) >= 0 and min5_kline.iloc[middle_index].close > min5_kline.iloc[-5-first_offset:middle_index].close.max() and min5_kline.iloc[middle_index].close > min5_kline.iloc[-5-first_offset:middle_index].open.max()
    if top_condition5:
    ...
    bottom_condition4 = min5_kline.iloc[last_index].close - min5_kline.iloc[last_index].open > 5 * price_tick and (min5_kline.iloc[middle_index].close > min5_kline.iloc[middle_index].open or min5_kline.iloc[last_index].close - min5_kline.iloc[last_index].open > 10 * price_tick) and min5_kline.iloc[first_index].open - min5_kline.iloc[first_index].close >= 0 and ma5.ma.iloc[first_index] > ma10.ma.iloc[first_index]
    # 明显的底分型快速开仓
    bottom_condition5 = min5_kline.iloc[last_index].close - min5_kline.iloc[last_index].open > 5 * price_tick and min5_kline.iloc[last_index].close > min5_kline.iloc[middle_index].open and min5_kline.iloc[middle_index].open - min5_kline.iloc[middle_index].close >= 0 and min5_kline.iloc[middle_index].close < min5_kline.iloc[-5-first_offset:middle_index].close.min() and min5_kline.iloc[middle_index].close < min5_kline.iloc[-5-first_offset:middle_index].open.min()
...
def cal_stop_signal():
    ...
    if long_entry_price > 0:
        ...
        if current_kline_form is not None and current_kline_form.type == KlineType.TOP:
            ...
            log(f'长趋势:{long_trend},中趋势:{middle_trend},短趋势:{short_trend},wait_period:{wait_period}到达2时还未盈利赶紧止损')
            stop_signal = 1
...
def target_profit_stop():
    ...
    if long_entry_price > 0:
        ...
        #     return
        # if (middle_trend > 0 or short_trend > 0) and ticks.iloc[-1].bid_price1 >= long_entry_price + big_profit_tick * price_tick:
        ...
        # if middle_trend < 0 and long_trend < 0:
        #     return
        # if (middle_trend < 0 or short_trend < 0) and ticks.iloc[-1].ask_price1 <= short_entry_price - big_profit_tick * price_tick:
        #     log(f'长趋势:{long_trend},中趋势:{middle_trend},短趋势:{short_trend},到达趋势止盈价格平仓')
        #     stop_signal = -1
...
def trySendOrder(volume, KC, PC):
    global long_entry_price, short_entry_price, wait_period, max_profit, last_kline_form
    # 平仓发单
    if (PC == 1):
