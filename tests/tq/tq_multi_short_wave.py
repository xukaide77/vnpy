...
logger = setup_logger(file_name=os.path.join(os.path.dirname(__file__), 'in_day_trade_short_wave'),
                           name='trade_pp',
                           log_level=logging.DEBUG,
                           backtesing=True)
price_logger = setup_logger(file_name=os.path.join(os.path.dirname(__file__), 'in_day_price_short_wave'),
                      name='price_pp',
                      log_level=logging.DEBUG,
                      backtesing=True)
...
api = TqApi(TqSim(10000), backtest=TqBacktest(start_dt=date(2020, 11, 8), end_dt=date(2020, 11, 14)), web_gui=True, auth="xukaide77,xk82513994")
# api = TqApi(TqSim(10000), backtest=TqBacktest(start_dt=datetime(2020, 11, 12, 9, 0), end_dt=datetime(2020, 11, 12, 15, 0)), web_gui=True, auth="xukaide77,xk82513994")
...
def CoreModule():
    ...
    if open_signal != 0:
        last_kline_form = None
        last_open_kline_form = current_kline_form
...
def cal_open_signal():
    ...
    if (now_hour == 9 and now_minute == 30) or (now_hour == 21 and now_minute == 30):
        ...
        middle_trend = 1 if ma40.ma.iloc[-1] > ma40.ma.iloc[-2] else -1
        if ma5.ma.iloc[-1] > ma10.ma.iloc[-1] > ma20.ma.iloc[-1]:
            # 向上趋势
            short_trend = 1
        elif ma5.ma.iloc[-1] < ma10.ma.iloc[-1] < ma20.ma.iloc[-1]:
            # 向下趋势
            short_trend = -1
        else:
            # 震荡
    ...
    if open_ready_price > 0:
        ...
        open_ready_price = 0
    if ticks.iloc[-1].bid_price1 > ma20.ma.iloc[-1]:
        ...
        if temp_ready_price > 0:
            if ticks.iloc[-1].bid_price1 < temp_ready_price and ticks.iloc[-1].bid_price1 > last_kline_form.stop_price:
                open_signal = 1
                long_stop_price = last_kline_form.stop_price
                short_stop_price = 0
                log(f'长趋势:{long_trend},中趋势:{middle_trend},短趋势:{short_trend},上涨趋势回调到位开多仓止损位至{long_stop_price}')
                return
            else:
                last_kline_form = None
        if current_kline_form is not None and current_kline_form.direction == Direction.LONG:
            if long_entry_price > 0:
                long_stop_price = current_kline_form.stop_price
                return
            ...
            log(f'长趋势:{long_trend},中趋势:{middle_trend},短趋势:{short_trend},stop_condition:{stop_condition},kline_form.stop_price:{current_kline_form.stop_price},ma:{ma20.ma.iloc[-1]}')
            # if current_kline_form.type == KlineType.CALLBACK_BOTTOM and middle_trend < 0:
            #     return
            if stop_condition: # or (price_int_time == 2118 or price_int_time == 918):
                open_signal = 1
                # stop_price = max(min(current_kline_form.stop_price, ticks.iloc[-1].ask_price1 - 0.7 * loss_tick * price_tick), ma20.ma.iloc[-1])
                stop_price = min(current_kline_form.stop_price, ticks.iloc[-1].ask_price1 - 0.7 * loss_tick * price_tick)
                log(f'长趋势:{long_trend},中趋势:{middle_trend},段趋势:{short_trend},上涨趋势设置开多仓止损位至{stop_price}')
                long_stop_price = stop_price
                short_stop_price = 0
            else:
                if current_kline_form.type == KlineType.CALLBACK_BOTTOM:
                    return
                open_ready_price = current_kline_form.stop_price + loss_tick * price_tick
                log(f'长趋势:{long_trend},中趋势:{middle_trend},段趋势:{short_trend}，准备开多时止损太大等回调价{open_ready_price}')
        elif current_kline_form is None and last_kline_form is not None and last_kline_form.direction == Direction.LONG:
            time_condition = last_kline_form.timestamp + trade_period * time_limit > price_int_time
            stop_condition = (ticks.iloc[-1].ask_price1 - loss_tick * price_tick) < last_kline_form.stop_price# or (ticks.iloc[-1].ask_price1 - loss_tick * price_tick) < ma20.ma.iloc[-1]
            price_condition = min5_kline.iloc[-1].close > min5_kline.iloc[-1].open
            log(f'长趋势:{long_trend},中趋势:{middle_trend},短趋势:{short_trend},time_condition:{time_condition},stop_condition:{stop_condition},price_condition:{price_condition},ma20:{ma20.ma.iloc[-1]}')
            if ((time_condition and stop_condition)) and price_condition:
                ...
                #     return
                stop_price = max(ticks.iloc[-1].ask_price1 - 0.7 * loss_tick * price_tick, ma20.ma.iloc[-1])
                open_signal = 1
                ...
                last_kline_form = None
    elif ticks.iloc[-1].ask_price1 < ma20.ma.iloc[-1]:
        # if not (short_trend < 0 or (middle_trend < 0 and short_trend == 0)):
        #     # 做单边
        #     return
        if ma20.ma.iloc[-1] > ma40.ma.iloc[-1] or ma40.ma.iloc[-1] > ma40.ma.iloc[-2]:
            return
        if temp_ready_price > 0:
            if ticks.iloc[-1].ask_price1 > temp_ready_price and ticks.iloc[-1].ask_price1 < last_kline_form.stop_price:
                open_signal = -1
                short_stop_price = last_kline_form.stop_price
                long_stop_price = 0
        ...
        if current_kline_form is not None and current_kline_form.direction == Direction.SHORT:
            if short_entry_price > 0:
                short_stop_price = current_kline_form.stop_price
                return
            stop_condition = (ticks.iloc[-1].bid_price1 + loss_tick * price_tick) > current_kline_form.stop_price# or (ticks.iloc[-1].bid_price1 + loss_tick * price_tick) > ma20.ma.iloc[-1]
            log(f'长趋势:{long_trend},中趋势:{middle_trend},短趋势:{short_trend},stop_condition:{stop_condition},kline_form.stop_price:{current_kline_form.stop_price},ma20:{ma20.ma.iloc[-1]}')
            # if current_kline_form.type == KlineType.CALLBACK_TOP and middle_trend > 0:
            #     return
            if stop_condition: # or (price_int_time == 2118 or price_int_time == 918):
                open_signal = -1
                # stop_price = min(max(ticks.iloc[-1].bid_price1 + 0.7 * loss_tick * price_tick, current_kline_form.stop_price), ma20.ma.iloc[-1])
                stop_price = max(ticks.iloc[-1].bid_price1 + 0.7 * loss_tick * price_tick, current_kline_form.stop_price)
                log('下跌趋势设置开空仓止损位至{}'.format(stop_price))
                short_stop_price = stop_price
                long_stop_price = 0
            else:
                if current_kline_form.type == KlineType.CALLBACK_TOP:
                    return
                open_ready_price = current_kline_form.stop_price - loss_tick * price_tick
                log(f'准备开空时止损太大等回调价{open_ready_price}')
        elif current_kline_form is None and last_kline_form is not None and last_kline_form.direction == Direction.SHORT:
            time_condition = last_kline_form.timestamp + trade_period * time_limit > price_int_time
            stop_condition = (ticks.iloc[-1].bid_price1 + loss_tick * price_tick) > last_kline_form.stop_price# or (ticks.iloc[-1].bid_price1 + loss_tick * price_tick) > ma20.ma.iloc[-1]
            price_condition = min5_kline.iloc[-1].close < min5_kline.iloc[-1].open
            log(f'长趋势:{long_trend},中趋势:{middle_trend},短趋势:{short_trend},time_condition:{time_condition},stop_condition:{stop_condition},price_condition:{price_condition},ma20:{ma20.ma.iloc[-1]}')
            if ((time_condition and stop_condition)) and price_condition:
                # if last_kline_form.type == KlineType.CALLBACK_TOP and middle_trend > 0:
                #     return
                stop_price = min(ticks.iloc[-1].bid_price1 + 0.7 * loss_tick * price_tick, ma20.ma.iloc[-1])
                open_signal = -1
                log('下跌趋势设置开空仓止损位至{}'.format(stop_price))
                short_stop_price = stop_price
                long_stop_price = 0
...
def top_or_bottom(int_time, ma5, ma10, ma20):
    ...
    top_condition1 = min5_kline.iloc[last_index].open > min5_kline.iloc[last_index].close and min5_kline.iloc[middle_index].close < min5_kline.iloc[middle_index].open and (min5_kline.iloc[first_index].close - min5_kline.iloc[first_index].open) >= 0 and (min5_kline.iloc[last_index].volume > min5_kline.iloc[middle_index].volume or min5_kline.iloc[last_index].open - min5_kline.iloc[last_index].close > min5_kline.iloc[middle_index].open - min5_kline.iloc[middle_index].close)# and min5_kline.iloc[first_index].close > min5_kline.iloc[-5-first_offset-second_offset:first_index].close.max()
    top_condition2 = min5_kline.iloc[last_index].open > min5_kline.iloc[last_index].close and (min5_kline.iloc[middle_index].close - min5_kline.iloc[middle_index].open) >= 0 and min5_kline.iloc[last_index].close < min5_kline.iloc[first_index].open < min5_kline.iloc[first_index].close and min5_kline.iloc[middle_index].close > min5_kline.iloc[-5-first_offset:middle_index].close.max() and min5_kline.iloc[last_index].open - min5_kline.iloc[last_index].close > 10 * price_tick and min5_kline.iloc[last_index].volume > min5_kline.iloc[middle_index].volume and min5_kline.iloc[last_index].volume > min5_kline.iloc[first_index].volume
    top_condition3 = min5_kline.iloc[last_index].open - min5_kline.iloc[last_index].close > 5 * price_tick and (min5_kline.iloc[middle_index].close - min5_kline.iloc[middle_index].open) >= 0 and min5_kline.iloc[last_index].close < min5_kline.iloc[first_index].close < min5_kline.iloc[first_index].open and ma5.ma.iloc[middle_index] < ma5.ma.iloc[middle_index-1] and (min5_kline.iloc[last_index].volume > min5_kline.iloc[first_index].volume or min5_kline.iloc[last_index].open - min5_kline.iloc[last_index].close > min5_kline.iloc[first_index].open - min5_kline.iloc[first_index].close) #min5_kline.iloc[middle_index].close > min5_kline.iloc[-4-first_offset:middle_index].close.max()
    ...
    if top_condition5:
        stop_price = min5_kline.iloc[last_index].open
        ...
        return KlineForm(stop_price, KlineType.TOP, can_time(top_datetime.hour, top_datetime.minute), Direction.SHORT)
    elif top_condition1:
    ...
    #     return KlineForm(stop_price, KlineType.CALLBACK_TOP, can_time(top_datetime.hour, top_datetime.minute), Direction.SHORT)
        ...
        return KlineForm(stop_price, KlineType.CALLBACK_TOP, can_time(top_datetime.hour, top_datetime.minute), Direction.SHORT)
    bottom_condition1 = min5_kline.iloc[last_index].close > min5_kline.iloc[last_index].open and min5_kline.iloc[middle_index].close > min5_kline.iloc[middle_index].open and min5_kline.iloc[first_index].open - min5_kline.iloc[first_index].close >= 0 and (min5_kline.iloc[last_index].volume > min5_kline.iloc[middle_index].volume or min5_kline.iloc[last_index].close - min5_kline.iloc[last_index].open > min5_kline.iloc[middle_index].close - min5_kline.iloc[middle_index].open)#and min5_kline.iloc[first_index].close < min5_kline.iloc[-5-first_offset-second_offset:first_index].close.min()
    bottom_condition2 = min5_kline.iloc[last_index].close > min5_kline.iloc[last_index].open and min5_kline.iloc[middle_index].open - min5_kline.iloc[middle_index].close >= 0 and min5_kline.iloc[last_index].close > min5_kline.iloc[first_index].open > min5_kline.iloc[first_index].close and min5_kline.iloc[middle_index].close < min5_kline.iloc[-5-first_offset:middle_index].close.min() and min5_kline.iloc[last_index].close - min5_kline.iloc[last_index].open > 10 * price_tick and min5_kline.iloc[last_index].volume > min5_kline.iloc[middle_index].volume and min5_kline.iloc[last_index].volume > min5_kline.iloc[first_index].volume
    bottom_condition3 = min5_kline.iloc[last_index].close - min5_kline.iloc[last_index].open > 5 * price_tick and min5_kline.iloc[middle_index].open - min5_kline.iloc[middle_index].close >= 0 and min5_kline.iloc[last_index].close > min5_kline.iloc[first_index].close > min5_kline.iloc[first_index].open and ma5.ma.iloc[middle_index] > ma5.ma.iloc[middle_index-1] and (min5_kline.iloc[last_index].volume > min5_kline.iloc[first_index].volume or min5_kline.iloc[last_index].close - min5_kline.iloc[last_index].open > min5_kline.iloc[first_index].close - min5_kline.iloc[first_index].open)  #and min5_kline.iloc[middle_index].close < min5_kline.iloc[-4-first_offset:middle_index].close.min()
    ...
    # 明显的底分型快速开仓
    bottom_condition5 = ((min5_kline.iloc[last_index].close - min5_kline.iloc[last_index].open > 10 * price_tick and min5_kline.iloc[last_index].close > min5_kline.iloc[middle_index].open) or min5_kline.iloc[last_index].close - min5_kline.iloc[last_index].open > 0.7 * loss_tick * price_tick) and min5_kline.iloc[middle_index].open - min5_kline.iloc[middle_index].close >= 0 and min5_kline.iloc[middle_index].close < min5_kline.iloc[-6-first_offset:middle_index].close.min() and min5_kline.iloc[middle_index].close < min5_kline.iloc[-6-first_offset:middle_index].open.min() and min5_kline.iloc[last_index].volume > min5_kline.iloc[middle_index].volume
    # 红三兵
    ...
    if bottom_condition5:
        ...
        return KlineForm(stop_price, KlineType.BOTTOM, can_time(bottom_datetime.hour, bottom_datetime.minute), Direction.LONG)
...
def cal_stop_signal():
    ...
    if long_entry_price > 0:
        ...
        #     stop_signal = 1
        #     log('做多突破MA20止盈')
        elif wait_period == max_wait_period:
            # 还未盈利就止损，已经盈利改变止损点
        ...
        #     stop_signal = 1
            ...
            stop_signal = 1
        elif min5_kline.iloc[-1].open > min5_kline.iloc[-1].close and min5_kline.iloc[-1].volume > 1.5 * min5_kline.iloc[-2].volume:
        ...
        #     log('做空突破MA20止盈')
            ...
            if ticks.iloc[-1].ask_price1 >= short_entry_price:
                log(f'长趋势:{long_trend},中趋势:{middle_trend},短趋势:{short_trend},wait_period:{wait_period}到达4时还未盈利赶紧止损')
                stop_signal = -1
            else:
                short_stop_price = short_entry_price
                log(f'长趋势:{long_trend},中趋势:{middle_trend},短趋势:{short_trend},wait_period:{wait_period}到达4时已经盈利改变止损点至开仓价{short_entry_price}')
        ...
        #     stop_signal = -1
            ...
            stop_signal = -1
        elif min5_kline.iloc[-1].close > min5_kline.iloc[-1].open and min5_kline.iloc[-1].volume > 1.5 * min5_kline.iloc[-2].volume:
