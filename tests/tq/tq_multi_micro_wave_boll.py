...
api = TqApi(TqSim(10000), backtest=TqBacktest(start_dt=date(2021, 1, 1), end_dt=date(2021, 1, 31)), web_gui=True, auth="xukaide77,xk82513994")
# api = TqApi(TqSim(10000), backtest=TqBacktest(start_dt=datetime(2020, 12, 9, 10, 30), end_dt=datetime(2020, 12, 9, 13, 0)), web_gui=True, auth="xukaide77,xk82513994")
...
# ----------------核心计算模块(每个bar更新一次)--------#
def CoreModule():
    global last_kline_form, current_kline_form, last_open_kline_form, open_long_sleep, open_short_sleep
    if last_kline_form is not None:
        last_kline_form = None
    cal_open_signal()
    cal_stop_signal()

    if open_long_sleep > 0:
        open_long_sleep -= 1
    if open_short_sleep > 0:
        open_short_sleep -= 1
...
def cal_open_signal():
    global long_stop_price, short_stop_price, open_signal, wait_period, short_trend, current_kline_form, shock_upper_limit, shock_down_limit, profit_time_left, open_ready_price, middle_trend, last_open_kline_form, last_kline_form, traded, open_long_sleep, open_short_sleep
    # 画一次指标线
    ...
    #     log(f'已经根据{last_open_kline_form.timestamp}信号开过仓')
        # return
    if (now_hour == 9 and now_minute <= 30) or (now_hour == 21 and now_minute <= 30) or (1445 <= int_time <= 1500) or (2245 <= int_time <= 2300):
        # 前分钟不开仓
        return 0
    if traded:
        log('本轮已经平仓，消除信号暂不开仓')
        ...
        traded = False
        return
    if current_kline_form is not None and current_kline_form.direction == Direction.LONG and min5_kline.iloc[-1].close_oi > min5_kline.iloc[-1].open_oi:
        ...
        #     return
            ...
            long_stop_price = current_kline_form.condition_open_price - loss_tick * price_tick + callback_ticks
            short_stop_price = 0
            open_long_sleep = 3
        ...
        #         log(f'设置开多仓条件价位至{last_kline_form.condition_open_price},止损位至{long_stop_price}')
            ...
            #     short_stop_price = 0
            #     log(f'设置开多仓条件价位至{last_kline_form.condition_open_price},止损位至{long_stop_price}')
    elif current_kline_form is not None and current_kline_form.direction == Direction.SHORT:
        # kdj = ta.KDJ(min5_kline, 9, 3, 3).j.iloc[-1]
        # if not (short_trend > 0 or (middle_trend > 0 and short_trend == 0)):
        #     # 做单边
        #     return
...
def top_or_bottom(int_time, ma5, ma10, ma20):
    ...
    if min5_kline.iloc[-1].open > boll.bottom.iloc[-1] and min5_kline.iloc[-1].close < boll.bottom.iloc[-1] and \
        (
            min5_kline.iloc[-1].open > ma20.ema.iloc[-1] or min5_kline.iloc[-2].open > ma20.ema.iloc[-2] or min5_kline.iloc[-3].open > ma20.ema.iloc[-3]
        ):
    ...
    elif min5_kline.iloc[-1].open < boll.top.iloc[-1] and min5_kline.iloc[-1].close > boll.top.iloc[-1] and \
        (
            min5_kline.iloc[-1].open < ma20.ema.iloc[-1] or min5_kline.iloc[-2].open < ma20.ema.iloc[-2] or min5_kline.iloc[-3].open < ma20.ema.iloc[-3]
        ):
...
def target_profit_stop():
    ...
    if long_entry_price > 0:
        ...
        #     return
        if ticks.iloc[-1].bid_price1 >= long_entry_price + profit_tick * price_tick:
            log(f'做多到达止盈价格平仓')
            stop_signal = 1
...
def trySendOrder(volume, KC, PC):
    global long_entry_price, short_entry_price, wait_period, max_profit, last_kline_form, traded, open_long_sleep, open_short_sleep
    # 平仓发单
    if (PC == 1):
...
def close_and_clearn():
    global is_trading, condition_open_long_price, condition_open_short_price, short_trend, last_kline_form, current_kline_form, shock_down_limit, shock_upper_limit, last_poc_price, profit_time_left, open_ready_price, last_open_kline_form, open_long_sleep, open_short_sleep
    if (is_trading == 1):
        trySendOrder(lots, 0, 1)
        trySendOrder(lots, 0, -1)
        is_trading = 0
